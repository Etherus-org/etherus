...
BUILD_FLAGS = -ldflags "-X github.com/ya-enot/ethermint/version.GitCommit=`git rev-parse --short HEAD`"
...
draw_deps:
	...
	goviz -i github.com/ya-enot/ethermint/cmd/ethermint -d 2 | dot -Tpng -o dependency-graph.png
...
get_vendor_deps:
	...
	@glide install
	@# ethereum/node.go:53:23: cannot use ctx (type *"github.com/ya-enot/ethermint/vendor/gopkg.in/urfave/cli.v1".Context) as type *"github.com/ya-enot/ethermint/vendor/github.com/ethereum/go-ethereum/vendor/gopkg.in/urfave/cli.v1".Context in argument to utils.SetEthConfig
...
docker_build_develop:
	docker build -t "ya-enot/ethermint:develop" -t "adrianbrink/ethermint:develop" \
		-f scripts/docker/Dockerfile.develop .

docker_push_develop:
	docker push "ya-enot/ethermint:develop"
	docker push "adrianbrink/ethermint:develop"

docker_build:
	docker build -t "ya-enot/ethermint" -t "ya-enot/ethermint:$(VERSION_TAG)" \
		-t "adrianbrink/ethermint" -t "adrianbrink/ethermint:$(VERSION_TAG)" -f scripts/docker/Dockerfile .

docker_push:
	docker push "ya-enot/ethermint:latest"
	docker push "ya-enot/ethermint:$(VERSION_TAG)"
